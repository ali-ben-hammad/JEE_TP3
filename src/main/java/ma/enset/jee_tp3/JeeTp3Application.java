...
public class JeeTp3Application {
   ...
   @Bean
    ...
    @Bean
    CommandLineRunner jdbcCommandLineRunner(JdbcUserDetailsManager jdbcUserDetailsManager){
       // PasswordEncoder passwordEncoder = passwordEncoder();
        return args -> {
            UserDetails user = jdbcUserDetailsManager.loadUserByUsername("user");
            if (user == null)
                jdbcUserDetailsManager.createUser(User.withUsername("user").password(passwordEncoder().encode("1234")).roles("USER").build());
